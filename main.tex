...
All of these rest on one assumption, that the spins form a long-range
ordered pattern, which will break time reversal in a specific way.
In this work we look for the adjacent regime. We take an altermagnetic system
and make its frustration so strong that quantum fluctuations
...
every site, even at zero temperature. The question is now: does anything
altermagnetic survive in a phase without spin order?

According to Sobral, Mandal, and Scheurer \cite{Sobral2025}, the answer is yes,
but the way it survives is subtle and requires new concepts. The altermagnetic
...
The question is not purely theoretical nature. Recent experiments indicate an interplay
...
especially the noncoplanar phase that will become central later.
...
magnetization is forced to vanish by a rotation instead of a translation. 
The following analysis is not limited to this particular model, it
...
Before walking through the phases we introduce the observables.
We do it in this sequence because when we later melt the ordered phases
...
\end{equation}
It is clearly invariant under global spin rotations. If all four components
...
\end{equation}
A scalar spin chirality requires three noncoplanar spins, vanishing
identically for every coplanar texture. Under time reversal all spins flip
sign, therefore a triple product of three flipped vectors acquires a minus sign, this makes 
$\chiEP$ odd under $\Theta$. It is the time-reversal detector in the
...
The anchor is the collinear altermagnet $A_1$, stabilized at small
...
true and because of the collinear texture the chirality vanishes. Modulo
...
Increasing $K$ gives a second-order transition into the canted altermagnet
$A_2$, which can be seen in \cref{fig:classical_transitions}(b). Both sublattices
develop the same out-of-plane tilt, producing a finite magnetization $M \neq 0$.
In the spin-rotation-invariant sector nothing changes as $\SEP$ stays uniform,
only with reduced magnitude, and theere is no chirality. 
At large $J_2$ one finds the orthogonal antiferromagnet $A_3$, a
...
remain uniform, so phase $C$ is not nematic. The new attribute is the chirality.
Because the texture is noncoplanar, the scalar spin chiralities are nonzero,
and the pattern around the plaquette is staggered,
\begin{equation}
    \label{eq:chipattern}
    \chiEP \propto (-1, 1, -1, 1).
\end{equation}
As shown in \cref{fig:classical_transitions}(a), phase $C$ is reached from $A_1$ by a
...
\begin{figure}[ht]
    ...
    continuously at the second-order transition from $A_1$ to $C$, this order holds aswell
    for the transition from $A_1$ to $A_2$. Adapted from
    Ref.~\cite{Sobral2025}.}
...
even at $T = 0$. Second, the ground state is long-range entangled as it cannot be
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.8\linewidth]{figures/anderson_rvb}
    \caption{Anderson's resonating valence bond (RVB) state, where the ellipsoids 
    represent the possible spin singlet pairings \cite{Savary2017}(p.2).}
    \label{fig:anderson_rvb}
\end{figure}
...
singlets, visualized in \cref{fig:anderson_rvb}. No single pairing dominates, 
so the state has no spin order by construction. One can also see that it cannot
be written as a product state over any finite region. 
Breaking one singlet does not create a local spin flip
...
Every spin operator is rewritten in terms of two bosons,
\begin{equation}
    \label{eq:sbrep}
...
\uparrow, \downarrow$ labels the boson flavor. This rewriting is exact as it
...
is a local U(1) gauge redundancy because the transformation $\hat{b}_{j\sigma} \to
...
site $i$ and one with spin down on site $j$, combined antisymmetrically. This makes
Anderson's valence bond more explicit. The hopping amplitude $B_{ij}$ is the
...
Ansätze systematically. We will come back to this strategy in \cref{sec:nearby}.
...
candidate. The gapped regime is the neighboring QSL, which inherits the IGG
...
isotropic, so $C_{4z}$ stays intact. However, the chirality correlator
stays staggered as $\langle \chiEP \rangle \propto (-1,1,-1,1)$. The time
reversality thereby remains broken in spin-rotation-invariant observables, with exactly
the $\Theta C_{4z}$ pattern of the orbital altermagnet.

There is no spin order anywhere in this phase, and
...
To summarize, first, the checkerboard $J_1$-$J_2$-$K$
...
beyond the scope of this work. The paper lays the foundation for the study of
